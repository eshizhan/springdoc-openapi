...
public class JavadocPropertyCustomizer implements ModelConverter {
     ...
     */
    void setJavadocDescription(Class<?> cls, List<Field> fields, List<PropertyDescriptor> clsProperties, Schema existingSchema) {
        if (existingSchema != null) {
            ...
            if (!CollectionUtils.isEmpty(properties)) {
                ...
                properties.entrySet().stream()
                        ...
                        .forEach(stringSchemaEntry -> {
                            fields.stream()
                                    .filter(field1 -> field1.getName().equals(stringSchemaEntry.getKey()))
                            ...
                            if (StringUtils.isBlank(stringSchemaEntry.getValue().getDescription())) {
                                clsProperties.stream()
                                        .filter(pd -> pd.getName().equals(stringSchemaEntry.getKey()))
                                        .findAny().ifPresent(pd1 -> {
            ...
            fields.stream().filter(f -> f.isAnnotationPresent(JsonUnwrapped.class))
                    .forEach(f -> setJavadocDescription(f.getType(), FieldUtils.getAllFieldsList(f.getType()), clsProperties, existingSchema));
